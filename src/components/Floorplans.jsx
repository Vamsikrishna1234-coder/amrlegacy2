...
const FloorPlansComponent = () => {
  ...
  const [selectedImage, setSelectedImage] = useState(null); // null = closed, string = image src
  ...
  const handleEastPrev = () => {
    setEastFacingIndex((prev) => (prev === 0 ? eastFacingPlans.length - 1 : prev - 1));
  };

  const handleEastNext = () => {
    setEastFacingIndex((prev) => (prev === eastFacingPlans.length - 1 ? 0 : prev + 1));
  };

  const handleWestPrev = () => {
    setWestFacingIndex((prev) => (prev === 0 ? westFacingPlans.length - 1 : prev - 1));
  };

  const handleWestNext = () => {
    setWestFacingIndex((prev) => (prev === westFacingPlans.length - 1 ? 0 : prev + 1));
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-22 min-h-screen">
      {/* Header */}
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold text-slate-800 mb-3">Floor Plans</h1>
        <p className="text-lg text-slate-600">Explore our East and West facing property layouts</p>
      </div>
      ...
      <div className="mb-16">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <h2 className="text-3xl font-bold text-slate-800">East Facing</h2>
        </div>

        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="relative">
            {/* Image Container */}
            <div className="relative aspect-video bg-slate-200 cursor-pointer" onClick={() => openModal(eastFacingPlans[eastFacingIndex].image)}>
              <img
              ...
              {/* Navigation Buttons */}
              <button
                onClick={(e) => { e.stopPropagation(); handleEastPrev(); }}
              ...
              <button
                onClick={(e) => { e.stopPropagation(); handleEastNext(); }}
      ...
      <div>
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <h2 className="text-3xl font-bold text-slate-800">West Facing</h2>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <div className="relative">
            ...
            <div 
              ...
              onClick={() => openModal(westFacingPlans[westFacingIndex].image)}
            >
              <img
              ...
              <button
                onClick={(e) => { e.stopPropagation(); handleWestPrev(); }}
                className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-3 rounded-full shadow-lg transition-all duration-200 hover:scale-110"
                aria-label="Previous"
              >
                <ChevronLeft className="w-6 h-6 text-slate-800" />
              ...
              <button
                onClick={(e) => { e.stopPropagation(); handleWestNext(); }}
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-3 rounded-full shadow-lg transition-all duration-200 hover:scale-110"
                aria-label="Next"
              >
                <ChevronRight className="w-6 h-6 text-slate-800" />
