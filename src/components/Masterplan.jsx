...
//    sqyd and sqft values matched exactly from the reference table.
...
//   ["03","241.5", "3425",   625, 974,  98, 85, "East"],
...
//   ["08","312.7", "3234",   338,1008,  98, 85, "East"],
...
//   ["44","337.5", "5127",  1044, 644,  92, 78, "West"],
//   ["45","337.5", "5127",  1111, 644,  92, 78, "West"],

//   /* ── Plots 46–47 (East facing) ─────────────────────────── */
//   ["46","337.5", "5104",  1111, 542,  92, 78, "East"],
//   ["47","337.5", "5104",  1044, 542,  92, 78, "East"],

//   /* ── FOURTH ROW: Plots 48 (East facing) ────────────────── */
//   ["48","300",   "4314",   782, 557,  78, 68, "East"],

//   /* ── Plots 49–57 (East facing) ─────────────────────────── */
//   ["49","300",   "4433",   731, 553,  78, 68, "East"],
//   ["50","300",   "4433",   672, 552,  78, 68, "East"],
//   ["51","300",   "4433",   614, 553,  78, 68, "East"],
//   ["52","300",   "4433",   558, 553,  78, 68, "East"],
//   ["53","300",   "4433",   501, 553,  78, 68, "East"],
//   ["54","300",   "4433",   443, 553,  78, 68, "East"],
//   ["55","300",   "4433",   387, 553,  78, 68, "East"],
//   ["56","300",   "4433",   330, 552,  78, 68, "East"],
//   ["57","300",   "4433",   273, 552,  78, 68, "East"],

//   /* ── Plot 58 (East facing) ─────────────────────────────── */
//   ["58","328.54","4790",   212, 553,  78, 68, "East"],

//   /* ── THIRD ROW: Plot 59 (West facing) ──────────────────── */
...
//   ["85","200",   "2757",   904, 354,  68, 58, "East"],
...
/* ================================================================
   🏷️  STATUS-VISIBLE VILLAS
   ─────────────────────────
   Only these villa numbers will show the Available / Sold badge
   and note in the tooltip. All other villas show tooltip info
   (Villa name, Area, Facing) but NO status label or note.

   To add/remove villas from this list, edit the set below.
================================================================ */
const STATUS_VISIBLE_IDS = new Set([
  "29","33","35","37","39","46","55","56",
  "59","71","73","74","96","97","107","108","109","111","112",
]);
...
const RAW = [
  ...
  /* ── FOURTH ROW: Plot 48 (East facing) ─────────────────── */
  ["48","300",   "4314",   782, 557,  78, 68, "East"],
...
const plots = RAW.map(([id, area, sqft, cx, cy, bw, bh, facing]) => ({
  ...
  ...b(+cx, +cy, +bw, +bh),
  status      : SOLD_IDS.has(id) ? "sold" : "available",
  showStatus  : STATUS_VISIBLE_IDS.has(String(parseInt(id, 10))),
  facing,
}));
...
function Tooltip({ plot, mx, my, containerWidth, containerHeight }) {
  if (!plot) return null;
  const cfg       = S[plot.status];
  const showBadge = plot.showStatus;

  const TOOLTIP_W = 240;
  const TOOLTIP_H = showBadge ? 155 : 115;
  ...
  const villaNum = parseInt(plot.id, 10);
  ...
  return (
    <div style={{
      ...
      {/* ── Header row ─────────────────────────────────────── */}
      <div style={{ display:"flex", justifyContent: showBadge ? "space-between" : "flex-end", marginBottom:8 }}>
        {showBadge && (
          <span style={{ fontWeight:600, fontSize:12, color:cfg.dot }}>
            {cfg.label}
          </span>
        )}
        <span style={{ fontWeight:700, fontSize:14 }}>Villa {villaNum}</span>
      </div>

      {/* ── Area ───────────────────────────────────────────── */}
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
        <span style={{ fontSize:12, color:"#252629" }}>Area</span>
        <span style={{ fontSize:12, fontWeight:600 }}>
          {plot.area} sq.yd
          <span style={{
            margin    : "0 6px",
      ...
      {/* ── Facing ─────────────────────────────────────────── */}
      <div style={{ display:"flex", justifyContent:"space-between" }}>
        <span style={{ fontSize:12, color:"#252629" }}>Facing</span>
        <span style={{ fontSize:12, fontWeight:600 }}>{ARR[plot.facing]} {plot.facing}</span>
      </div>

      {/* ── Status note (only for visible villas) ──────────── */}
      {showBadge && (
        <div style={{ marginTop:8, fontSize:11, color:cfg.dot, textAlign:"center" }}>
          {cfg.note}
        </div>
