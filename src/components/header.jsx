...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50">
      ...
      {/* ===================== TOP INFO BAR ===================== */}
      <div
        className={`${!scrolled ? "xl:block hidden" : "hidden"} 
        bg-[#d1a32c] text-white text-[15px] py-3 transition-all duration-300`}
      >
        <div className="max-w-7xl mx-auto px-3 flex justify-between items-center">
          <div className="flex items-center gap-9">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-3">
              <a
                href="https://www.facebook.com/amrlivingspaces"
                target="_blank"
                className="w-8 h-8 flex items-center justify-center rounded-full bg-white/20 border border-white/70 hover:bg-white/40 transition"
              >
                <Facebook size={16} className="text-white" />
              </a>
      ...
      <nav
        className={`transition-all duration-500 ${
          scrolled ? "bg-[#203370] shadow-lg" : "bg-black/40 backdrop-blur-[4px]"
        }`}
      >
        <div className="max-w-7xl mx-auto px-3 flex items-center justify-between py-3">
          ...
          <Link to="/" className="flex items-center gap-4">
            ...
            <img
              src={legacyLogo}
              alt="Legacy Logo"
              className="h-14 sm:h-16 md:h-22 object-contain"
            />
          </Link>

          {/* DESKTOP MENU NOW ONLY ON LARGE SCREENS */}
          <ul className="hidden lg:flex items-center gap-16 font-medium text-[17px] text-white">
            <li className="hover:text-[#d1a32c]"><Link to="/">Home</Link></li>
            <li className="hover:text-[#d1a32c]"><Link to="/about">About Us</Link></li>
            <li className="hover:text-[#d1a32c]"><Link to="/blogs">Blogs</Link></li>
            <li className="hover:text-[#d1a32c]"><Link to="/contact">Contact Us</Link></li>
          ...
          <button
            onClick={() => (window.location.href = "/contact")}
            className="hidden lg:block px-7 py-3 bg-[#d1a32c] text-white rounded-full 
                       font-semibold hover:bg-[#bb8c19] transition"
          >
            Enquiry Now
          </button>
        ...
        {/* MOBILE/TABLET MENU */}
        <div
          className={`lg:hidden bg-[#203370] text-white overflow-hidden transition-all duration-500 ${
            menuOpen ? "max-h-96 p-5" : "max-h-0 p-0"
          }`}
        >
          <ul className="flex flex-col gap-5 text-lg font-medium">
